...
class HTTPSServer(HTTPServer):
    def __init__(self, address, handler, pem_file):
        ...
        _soc = socket.socket(self.address_family, self.socket_type)
        self.socket = wrap_socket_in_ssl_connection(_soc, pem_file=self.pem_file)
...
#
# MITM PROXY HTTP REQUEST HANDLER AND SERVER as the title suggests, the
# following classes provide man-in-the-middle capabilities to
# ProxyHTTPRequestHandler and ProxyHTTPServer
#
class MitmProxyHTTPRequestHandler(ProxyHTTPRequestHandler):
    def do_GET(self):
        # chainedHandler is part of HTTPServerWrapper pseudo class and it is
        # used only here to identify if we are about to mitm the ssl connection
        if hasattr(self.server, 'chainedHandler'):
            # if yes, fix the path to include the https protocol and the
            # chainedHandler path
    ...
    def do_CONNECT(self):
        try:
            ...
            # however, here we reverse the connection and we wrap it in
            # SSLConnectionWrapper
            browser_soc = self.connection
            browser_con = wrap_socket_in_ssl_connection(self.connection,
                                                        pem_file=self.server.pem_file,
                                                        state='accept')
            wrapped_connection = SSLConnectionWrapper(browser_con, browser_soc)

            # start a pseudo HTTPServerWrapper object to handle the rest of the
            # communication
...
# the following classes define an observable MitmProxyHTTPRequestHandler and
# MitmProxyHTTPServer with a monkey patch
#
...
# these classes are 1337, as they define observable/tamperable
# MitmProxyHTTPRequestHandler and MitmProxyHTTPServer
#
class SimpleObservableProxyHTTPRequestHandler(MitmProxyHTTPRequestHandler):
    ...
    def _sock_read_write(self, soc, data=None, max_idling=20):
        ...
        # unfortunately file sockets are not supported by OpenSSL so we need to
        # go arround with a hack 
        try:
